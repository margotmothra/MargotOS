...
# Define directories
ZANEYOS_DIR="$HOME/margotos"
BACKUP_BASE_DIR="$HOME/.config/margotos-backups"
TIMESTAMP=$(date +"%Y-%m-%d_%H-%M-%S")
BACKUP_DIR="$BACKUP_BASE_DIR/margotos-2.3-upgrade-backup-$TIMESTAMP"

# Target branch for 2.4 release (override by exporting BRANCH)
BRANCH="${BRANCH:-stable-2.4}"

# Define log file
LOG_FILE="$HOME/margotos-upgrade-$TIMESTAMP.log"
...
create_backup() {
    ...
    print_info "Copying entire margotos directory..."
    if cp -r "$ZANEYOS_DIR" "$BACKUP_DIR/"; then
        print_success "Backup created successfully at: $BACKUP_DIR"
        echo -e "${GREEN}📁 Backup location: $BACKUP_DIR/margotos${NC}"
        return 0
    else
...
revert_from_backup() {
    ...
    if [ ! -d "$BACKUP_DIR/margotos" ]; then
        print_error "Backup not found at $BACKUP_DIR/margotos"
    ...
    echo -e "${YELLOW}Current margotos directory will be replaced with backup${NC}"
    ...
    print_info "Removing current margotos directory..."
    if [ -d "$ZANEYOS_DIR" ]; then
        rm -rf "$ZANEYOS_DIR"
    fi
    ...
    if cp -r "$BACKUP_DIR/margotos" "$HOME/"; then
        ...
        if command -v nh &> /dev/null; then
            nh os boot ~/margotos --hostname "$profile"
        else
            sudo nixos-rebuild boot --flake ~/margotos/#"$profile"
        fi
...
# Change to margotos directory
if [ ! -d "$ZANEYOS_DIR" ]; then
    print_error "ZaneyOS directory not found at $ZANEYOS_DIR"
    print_error "Please ensure ZaneyOS is installed at ~/margotos"
    exit 1
fi
...
perform_precheck_analysis() {
    local analysis_file="$HOME/margotos-upgrade-analysis-$TIMESTAMP.txt"
    ...
    
    {
        ...
        if [ -d ".git" ]; then
            ...
            if git remote get-url origin >/dev/null 2>&1; then
                ...
                if [[ "$remote_url" != *"zaney/margotos"* ]]; then
                    ...
                    echo "     ./compare-margotos-config.sh -r $remote_url -b $current_branch"
                fi
            fi
        ...
        echo "   ~/.config/margotos-backups/margotos-2.3-upgrade-backup-$TIMESTAMP/"
        echo ""
...
HOST_DIRS=()
if [ -d "$BACKUP_DIR/margotos/hosts" ]; then
    for dir in "$BACKUP_DIR/margotos/hosts"/*/; do
        [ -d "$dir" ] || continue
        hostname=$(basename "$dir")
        if [ "$hostname" != "default" ]; then
            HOST_DIRS+=("$hostname")
...
merge_variables() {
    local hostname=$1
    local old_vars_file="$BACKUP_DIR/margotos/hosts/$hostname/variables.nix"
    ...
    if [ -f "$BACKUP_DIR/margotos/hosts/$hostname/hardware.nix" ]; then
        cp "$BACKUP_DIR/margotos/hosts/$hostname/hardware.nix" "./hosts/$hostname/"
        print_success "✓ Preserved hardware.nix for host: $hostname"
    fi
    
    # Also copy host-packages.nix if it exists (user's custom packages)
    if [ -f "$BACKUP_DIR/margotos/hosts/$hostname/host-packages.nix" ]; then
        cp "$BACKUP_DIR/margotos/hosts/$hostname/host-packages.nix" "./hosts/$hostname/"
    ...
    if [ -f "$BACKUP_DIR/margotos/hosts/$hostname/default.nix" ]; then
        # Check if it's different from the template default.nix
        if ! cmp -s "$BACKUP_DIR/margotos/hosts/$hostname/default.nix" "./hosts/default/default.nix"; then
            cp "$BACKUP_DIR/margotos/hosts/$hostname/default.nix" "./hosts/$hostname/"
...
if [ -f "$BACKUP_DIR/margotos/modules/core/packages.nix" ]; then
    ...
    if ! cmp -s "$BACKUP_DIR/margotos/modules/core/packages.nix" "./modules/core/packages.nix"; then
        ...
        cp "$BACKUP_DIR/margotos/modules/core/packages.nix" "./modules/core/packages.nix"
...
update_flake_configuration() {
    ...
    if [ -n "${HOST_DIRS[0]}" ] && [ -f "$BACKUP_DIR/margotos/hosts/${HOST_DIRS[0]}/variables.nix" ]; then
        detected_username=$(grep 'gitUsername' "$BACKUP_DIR/margotos/hosts/${HOST_DIRS[0]}/variables.nix" | sed 's/.*= *"\([^"]*\)".*/\1/' | head -1)
    fi
...
if [ "$USE_NH" = true ]; then
    ...
    if nh os boot ~/margotos --hostname "$GPU_PROFILE"; then
        BUILD_SUCCESS=true
    fi
else
    print_info "Using nixos-rebuild for build..."
    if sudo nixos-rebuild boot --flake ~/margotos/#"$GPU_PROFILE"; then
        BUILD_SUCCESS=true
    fi
fi
