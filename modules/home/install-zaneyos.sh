...
print_success_banner() {
  echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${GREEN}║                 margotos Installation Successful!                      ║${NC}"
  echo -e "${GREEN}║                                                                       ║${NC}"
...
print_failure_banner() {
  echo -e "${RED}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${RED}║                 margotos Installation Failed!                          ║${NC}"
  echo -e "${RED}║                                                                       ║${NC}"
...
print_header "Backup Existing margotos (if any)"

backupname=$(date +"%Y-%m-%d-%H-%M-%S")
if [ -d "margotos" ]; then
  echo -e "${GREEN}margotos exists, backing up to .config/margotos-backups folder.${NC}"
  if [ -d ".config/margotos-backups" ]; then
    echo -e "${GREEN}Moving current version of margotos to backups folder.${NC}"
    mv "$HOME"/margotos .config/margotos-backups/"$backupname"
    sleep 1
  else
    echo -e "${GREEN}Creating the backups folder & moving margotos to it.${NC}"
    mkdir -p .config/margotos-backups
    mv "$HOME"/margotos .config/margotos-backups/"$backupname"
    sleep 1
  fi
else
  echo -e "${GREEN}Thank you for choosing margotos.${NC}"
  echo -e "${GREEN}I hope you find your time here enjoyable!${NC}"
fi

print_header "Cloning margotos Repository"
git clone https://gitlab.com/dwilliam62/margotos.git --depth=1  ~/margotos
cd ~/margotos || exit 1
